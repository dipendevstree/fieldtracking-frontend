...
export const useGetRolesAndPermissionById = (
  ...
  const query = useFetchData<any>({
    url: `${API.roles.getAllPermissions}/${roleId}`,
    enabled: (options?.enabled ?? true) && !!roleId, // Only enable if roleId present
  })

  const responseData = query.data
  console.log('responseData111111111', query.data)
  return {
    ...query,
    data: responseData ?? [],
    rolePermission: responseData || [],
