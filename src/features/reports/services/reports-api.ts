...
class ReportsAPI {
  ...
  async getExpenseReports(_filters?: ReportFilter): Promise<ExpenseReport[]> {
    ...
    return mockData.filter((report) => {
      if (
        _filters?.salesRep &&
        !report.salesRep.toLowerCase().includes(_filters.salesRep.toLowerCase())
      ) {
        return false;
      }
      if (
        ...
        !report.expense_category
          .toLowerCase()
          .includes(_filters.category.toLowerCase())
      ) {
        return false;
      }
      return true;
  ...
  async getFieldActivityReports(
    _filters?: ReportFilter
  ): Promise<FieldActivityReport[]> {
    return [
      {
        id: 1,
