...
interface DataTableRowActionsProps {
  row: {
    original: TrackingRule | GeofenceZone
  }
  type: 'rule' | 'zone'
}

export function DataTableRowActions({ row, type }: DataTableRowActionsProps) {
  const { setOpen, setCurrentRule, setCurrentZone } = useFieldTrackingStore()

  const handleEdit = () => {
    if (!row.original) return
    
    if (type === 'rule') {
      setCurrentRule(row.original as TrackingRule)
      setOpen('edit-rule')
    } else {
      setCurrentZone(row.original as GeofenceZone)
      setOpen('edit-zone')
    }
  }
