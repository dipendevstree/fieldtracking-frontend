...
import {
  ...
  MobileFeatureSettingsPayload
} from '../services/MobileFeatureshook'
import { useMobileFeaturesStore } from '../store/mobile-features.store'
import { MobileFeaturesConfigActionForm, MobilePermissionActionForm } from './action-form'
import { TMobileFeaturesConfigFormSchema, TMobilePermissionFormSchema, TMobileFeatureSettingsFormSchema } from '../data/schema'
...
export function MobileFeaturesActionModal() {
  ...
  const {
    mutate: createMobileFeatureSettings,
  ...
  const {
    mutate: updateMobileFeatureSettings,
  ...
  const {
    mutate: deleteMobileFeatureSettings,
  ...
  const handleUpdateMobileFeaturesConfig = (values: TMobileFeaturesConfigFormSchema) => {
    try {
      const payload: MobileFeaturesConfigPayload = {
        scheduleAndReports: {
          allowScheduleViewing: values.scheduleAndReports.allowScheduleViewing,
          allowVisitSummaryAccess: values.scheduleAndReports.allowVisitSummaryAccess,
          allowExpenseReportAccess: values.scheduleAndReports.allowExpenseReportAccess,
        },
        cameraAndPhoto: {
  ...
  const handleCreateMobilePermission = (values: TMobilePermissionFormSchema) => {
    try {
  ...
  const handleUpdateMobilePermission = (values: TMobilePermissionFormSchema) => {
    try {
  ...
  // Mobile Feature Settings handlers
  const handleCreateMobileFeatureSettings = (values: TMobileFeatureSettingsFormSchema) => {
    try {
      const payload: MobileFeatureSettingsPayload = {
      ...
      createMobileFeatureSettings(payload)
    } catch (error) {
      console.error('Error creating mobile feature settings:', error)
      toast.error('Failed to create mobile feature settings')
    }
  }

  const handleUpdateMobileFeatureSettings = (values: TMobileFeatureSettingsFormSchema) => {
    try {
      if (!currentFeature?.settingsId) {
        toast.error('Feature settings ID is missing')
        return
      }
      ...
      updateMobileFeatureSettings(payload)
    } catch (error) {
      console.error('Error updating mobile feature settings:', error)
      toast.error('Failed to update mobile feature settings')
    }
  }

  const handleDeleteMobileFeatureSettings = () => {
    try {
      if (!currentFeature?.settingsId) {
        toast.error('Feature settings ID is missing')
        return
      ...
      deleteMobileFeatureSettings()
    } catch (error) {
      console.error('Error deleting mobile feature settings:', error)
      toast.error('Failed to delete mobile feature settings')
    }
  }
