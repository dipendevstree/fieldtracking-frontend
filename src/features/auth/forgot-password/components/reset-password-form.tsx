...
import { z } from "zod";
import { useResetPassword } from "../services/forgot-services";
...
export function ResetPasswordForm() {
  ...
  const { token }: any = useSearch({
    from: "/(auth)/reset-password",
  });

  const { navigate } = useRouter();

  const { mutate: setPassword, isPending: isLoading } = useResetPassword(() => {
    navigate({ to: "/sign-in" });
  });
  ...
  const onSubmit = (data: SetPasswordValues) => {
    setError("");
    setPassword({ new_password: data.password, token });
  };
