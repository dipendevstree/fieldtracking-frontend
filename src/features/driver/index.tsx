...
import { ALL_PAGE_META_DATA, DEFAULT_PAGE_NUMBER, DEFAULT_PAGE_SIZE } from "@/data/app.data"
...
const DriverPage = () => {
    ...
    const [pagination, setPagination] = useState({ page: DEFAULT_PAGE_NUMBER, limit: DEFAULT_PAGE_SIZE });
    const { totalCount = 0, listData = [], isLoading, error } = useGetDrivers(pagination)
    ...
    if (error) {
        ...
        return <ErrorPage errorCode={errorResponse?.statusCode} message={errorResponse?.message} />;
    }
